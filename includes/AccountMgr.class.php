	...
	static function getHistory($guid, $full = true)
	{
		...
		while ($row = mysql_fetch_row($result))
		{
			...
			if ($date >= date('Y-m-d H:i:s', time() - 15778463 /* 6 Monate */))
			{
				...
				$html .= "<tr style=\"background-image: url('images/warning.gif');\">";
				$html .= "<td>". $row[0] . "</td>";
				$html .= "<td>" . $wirkung[$row[1]-1] . "</td>";
				$html .= "<td>" . $row[2] . "</td>";
				$html .= "<td>" . $row[3] . "</td>";
		...
		if (self::getLastVerwarnstufe($guid) < 5)
			$html .= "<tr><td>*</td><td>". self::buildStufenSelect(self::getLastVerwarnstufe($guid)) ."</td><td><textarea name=\"kommentar\" maxlength=\"255\"/></textarea></td><td>*</td><td><font color=\"".TextMgr::getText($_SESSION['rank'], false)."\">".$_SESSION['username']."</font></td></tr>";
		$html .= "</tbody></table>";
		return($html);
	}
